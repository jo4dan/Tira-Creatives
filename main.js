...
const prefersReduced = window.matchMedia('(prefers-reduced-motion: reduce)'); // keep only this one [no duplicate]
...
document.querySelectorAll('a[href^="#"]').forEach((a) => {
  a.addEventListener('click', (e) => {
    ...
    if (!el) return;
    e.preventDefault();
    const top = el.offsetTop - 64; // offset for fixed navbar
...
document.querySelectorAll('.service-card,.portfolio-item').forEach((el, i) => {
  ...
  el.style.transition = `all .6s cubic-bezier(.4,0,.2,1) ${i * 0.08}s`;
  io.observe(el);
});
...
if (isMobile() && !prefersReduced.matches) {
  ...
  const addRippleTo = (sel) =>
    document.querySelectorAll(sel).forEach((el) => el.addEventListener('touchstart', ripple, { passive: true }));
  addRippleTo('.cta-button');
  addRippleTo('.submit-button');
  addRippleTo('.service-card');
...
/* Light feature gating clean-up for phones / reduced motion */
const effectsOn = () => !prefersReduced.matches && !isMobile();
if (!effectsOn()) {
  ...
  if (spray) {
    ...
    if (ctx) ctx.clearRect(0, 0, spray.width, spray.height);
  }
}
